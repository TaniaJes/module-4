...
CountTable <- read.table("kallisto_counts.txt", header=TRUE, row.names=1) 
#samples <- data.frame(colnames(CountTable))
...
Dataset$condition <- relevel(Dataset$condition, "siCTRL-12h")
...
res1 <- lfcShrink(DatasetProcessed, contrast=c("condition","siCTRL-12h-dsDNA","siCTRL-12h"))
...
baseMeanCtrl = rowMeans(counts(DatasetProcessed,normalized=TRUE)[,DatasetProcessed$condition == "siCTRL-12h"])
baseMeanCnot = rowMeans(counts(DatasetProcessed,normalized=TRUE)[,DatasetProcessed$condition == "siCTRL-12h-dsDNA"])
...
subset <- head((subset(res1, res1$padj < 0.01)), n=10000000000)  
subset1 <- subset[order(subset$log2FoldChange,decreasing=T),]
subset2 <- subset[order(subset$log2FoldChange,decreasing=F),]
#subset <- rbind(subset1[1:12,],subset2[1:12,])
subset <- subset1[1:30,]
