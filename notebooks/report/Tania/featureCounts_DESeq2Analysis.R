...
featCounts <- read.table("./featureCounts_output/geneCounts-output.txt", header = T, row.names = 1)
...
# out of 16041 with nonzero total read count
# adjusted p-value < 0.01
# LFC > 1.00 (up)    : 1132, 7.1%
# LFC < -1.00 (down) : 572, 3.6%
# outliers [1]       : 0, 0%
# low counts [2]     : 1831, 11%
# (mean count < 2)
...
write.csv(res1, "featureCounts_DESeq2.csv", row.names=TRUE)
...
EnhancedVolcano(toptable = res1, x = "log2FoldChange", y = "pvalue",
                lab = res1$symbol, pCutoff = 10e-6, FCcutoff = 1, xlim = c(-5, 5), ylim = c(-20, 350),
                title = "featureCounts DESeq2 results", subtitle = "Differential Expression Analysis", caption = NULL,
...
pheatmap(mat, cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=TRUE, annotation_col=df, main = "featureCounts - Top 25 most significant genes") 
